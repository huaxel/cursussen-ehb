...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelector('body').insertAdjacentHTML('afterbegin', '<h1>Leerplatform</h1>');
    const courses = [];
    const students = [];
        ...
        studentListElement.innerHTML = students
            .map(
                (student) =>
                    `<div>${
                        student.name
                    } - Ingeschreven in: ${student.enrolledCourses.join(
                        ", "
                    )}</div>`
            )
            .join("");
    ...
    generateReportButton.addEventListener("click", function () {
        ...
        if (!student) {
            reportOutputElement.innerHTML =
                "<div>Geen student geselecteerd.</div>";
            return;
        }
        ...
        const report =
            Object.entries(student.scores)
                .map(
                    ([course, grades]) =>
                        `<div><strong>${course}:</strong> ${grades.join(
                            ", "
                        )}</div>`
                )
